...
resource "aws_s3_bucket" "foo" {
  // 23. AWS S3 buckets are accessible to public
  ...
  bucket = "foo_name"
  // 24. AWS S3 Object Versioning is disabled
  ...
  // ??? [UNABLE TO MATCH] 1. AWS S3 CloudTrail buckets for which access logging is disabled ()
...
resource "aws_cloudtrail" "foo_cloudtrail" {
  // 2. AWS CloudTrail bucket is publicly accessible
...
resource "aws_vpc_peering_connection" "foo" {
  peer_vpc_id = aws_vpc.foo_vpc.id
...
// (Bug??? cidr_blocks contains does not match) 27. AWS Security Groups with Inbound rule overly permissive to All Traffic
...
# Modified [aws_all_issues_addressed.tf] on [2020-11-30 23:01:07.554757]